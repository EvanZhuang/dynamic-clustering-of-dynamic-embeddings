...
df = pd.read_csv("~/PycharmProjects/dynamic-clustering\
    -of-dynamic-embeddings/dat/un-general-debates.csv")
trained_topics = []
for i in range(10):
    with open('/home/yufan/PycharmProjects/dynamic-clustering-of-dynamic\
      -embeddings/src/topic_'+str(i)+'.txt') as f:
...
cv2 = gensim.models.CoherenceModel(
    topics=trained_topics, texts=doc_clean, dictionary=dictionary,
    coherence='c_v', window_size=110)
umass2 = gensim.models.CoherenceModel(
    topics=trained_topics, corpus=doc_term_matrix, dictionary=dictionary,
    coherence='u_mass')
cuci2 = gensim.models.CoherenceModel(
    topics=trained_topics, texts=doc_clean, dictionary=dictionary,
    coherence='c_uci', window_size=10)
cnpmi2 = gensim.models.CoherenceModel(
    topics=trained_topics, texts=doc_clean, dictionary=dictionary,
...
ntopic = 10
ldamodel = Lda(doc_term_matrix, num_topics=ntopic,
               id2word=dictionary, passes=1,
               workers=multiprocessing.cpu_count()-1,
               eval_every=False, chunksize=1000)
...
N = 10
cv = gensim.models.CoherenceModel(
    model=ldamodel, texts=doc_clean, dictionary=dictionary,
    coherence='c_v', topn=N, window_size=110)
umass = gensim.models.CoherenceModel(
    model=ldamodel, corpus=doc_term_matrix, dictionary=dictionary,
    coherence='u_mass', topn=N)
cuci = gensim.models.CoherenceModel(
    model=ldamodel, texts=doc_clean, dictionary=dictionary,
    coherence='c_uci', topn=N, window_size=10)
cnpmi = gensim.models.CoherenceModel(
    model=ldamodel, texts=doc_clean, dictionary=dictionary,
