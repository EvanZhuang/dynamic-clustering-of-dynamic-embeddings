...
class bern_emb_data():
    def __init__(self, cs, ns, fpath, dynamic, n_epochs=1,
        ...
        dat_stats = pickle.load(open(os.path.join(
            fpath, "dat_stats.pkl"), "rb"))
        ...
        if not self.dynamic:
            ...
            self.N = np.sum(dat_stats['train']).astype('int32')
            self.n_train = np.maximum(
                dat_stats['train']/n_epochs, 2*np.ones_like(
                    dat_stats['train'])).astype('int32')
            self.n_valid = dat_stats['valid'].astype('int32')
            self.n_test = dat_stats['test'].astype('int32')

    # load vocabulary
        df = pd.read_csv(os.path.join(fpath, 'unigram.txt'),
        ...
        if remove_stopwords:
            sw_df = pd.read_csv(os.path.join(fpath, 'stop_words.txt'),
                                delimiter='\t', header=None)
            stop_words = sw_df[0].values
            self.w_idx = [i for i, w in enumerate(self.labels)
                          if w not in stop_words]
            self.labels = self.labels[self.w_idx]
            self.counts = self.counts[self.w_idx]
            self.unigram = self.unigram[self.w_idx]
            self.unigram_t = np.load(os.path.join(
                fpath, 'unigram_t.npy'))[:, self.w_idx]
        ...
        self.query_words = [w for w in dat_stats['query_words']
                            if w in self.labels]
        ...
        if self.dynamic:
            ...
            for t, i in enumerate(dat_stats['T_bins']):
                self.batch[t] = self.batch_generator(
                    self.n_train[t] + self.cs,
                    [f for f in train_files if
                     int(os.path.basename(f)[:dat_stats['prefix']]) == i])
        else:
            self.batch = self.batch_generator(
                self.n_train + self.cs, train_files)
        ...
        if self.dynamic:
            ...
            for t, i in enumerate(dat_stats['T_bins']):
                self.valid_batch[t] = self.batch_generator(
                    self.n_valid[t] + self.cs, [
                        f for f in valid_files if
                        int(os.path.basename(f)[:dat_stats['prefix']]) == i])
        else:
            self.valid_batch = self.batch_generator(
                self.n_valid + self.cs, valid_files)
        ...
        if self.dynamic:
            ...
            for t, i in enumerate(dat_stats['T_bins']):
                self.test_batch[t] = self.batch_generator(
                    self.n_test[t] + self.cs, [
                        f for f in test_files if int(
                            os.path.basename(f)[:dat_stats['prefix']]) == i])
        else:
            self.test_batch = self.batch_generator(
                self.n_test + self.cs, test_files)
